class ToDo {
    ...
    loadTasks(){
        return JSON.parse(localStorage.getItem(this.storageKeys))
    }
    ...
    addTask(text,) {
        const newTaskDate = { text: text, isCompleted: false }
        const newTasks = this.tasks.concat(newTaskDate)
        this.setTasks(newTasks)
    }
    ...
    renderTasks() {
        this.tasks.forEach((taskData, index) => {
            const task = new Task(taskData,()=> this.toggleComplete(index), ()=> this.deleteTask(index))
